...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const payload: Record<string, unknown> = {
    model: ENV.openRouterApiKey ? "openrouter/free" : "gemini-2.5-flash",
...
export function extractAssistantText(result: InvokeResult): string {
  ...
  if (Array.isArray(c)) {
    const textPart = c.find((x): x is TextContent => (x as TextContent).type === "text");
    return textPart?.text ?? "";
  }
  return "";
}

/** Yields small chunks for SSE streaming (token-like UX when API has no native stream). */
export async function* streamAssistantChunks(params: InvokeParams): AsyncGenerator<string> {
